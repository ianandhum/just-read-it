...
describe('createReadingSession', () => {
  ...
  // --- coverage (mouse read rule) -------------------------------------------
  // The first setProgress() after setCurrent() latches the entry position.
  // To simulate "enter at X", call setProgress(X) first; subsequent calls
  // sweep from there.

  it('getMaxProgress reports coverage relative to the entry position', () => {
    // Enter at 0 and sweep to 0.7 → coverage 0.7.
    const doc = setupDoc();
    const spans = makeSpans(doc, [0]);
    const session = createReadingSession(spans, null);
    session.setCurrent(0);
    session.setProgress(0); // entry mousemove at the start
    session.setProgress(0.7); // sweep forward
    expect(session.getMaxProgress()).toBeCloseTo(0.7, 5);
    session.destroy();
  });

  it('entering near the end covers ~0 (no fake read)', () => {
    // Enter at 0.95 and wiggle to 0.98 → coverage 0.03, NOT 0.98.
    const doc = setupDoc();
    const spans = makeSpans(doc, [0]);
    const session = createReadingSession(spans, null);
    session.setCurrent(0);
    session.setProgress(0.95); // entry mousemove at 0.95
    expect(session.getMaxProgress()).toBeCloseTo(0, 5); // 0.95 - 0.95
    session.setProgress(0.98);
    expect(session.getMaxProgress()).toBeCloseTo(0.03, 5); // 0.98 - 0.95
    session.destroy();
  });

  it('moving backward does not reduce coverage', () => {
    const doc = setupDoc();
    const spans = makeSpans(doc, [0]);
    const session = createReadingSession(spans, null);
    session.setCurrent(0);
    session.setProgress(0); // enter at start
    session.setProgress(0.8); // coverage 0.8
    session.setProgress(0.2); // back up — coverage stays 0.8
    expect(session.getMaxProgress()).toBeCloseTo(0.8, 5);
    session.destroy();
  });

  it('re-entering mid-sentence resets coverage from the new entry', () => {
    // Enter at 0, sweep to 0.5, leave, re-enter at 0.5, sweep to 1.0 → 0.5.
    const doc = setupDoc();
    const spans = makeSpans(doc, [0, 1]);
    const session = createReadingSession(spans, null);
    session.setCurrent(0);
    session.setProgress(0); // enter at start
    session.setProgress(0.5); // coverage 0.5
    expect(session.getMaxProgress()).toBeCloseTo(0.5, 5);
    session.setCurrent(1); // leave
    session.setCurrent(0); // re-enter; resumes at 0.5, entryPending=true
    expect(session.getMaxProgress()).toBeCloseTo(0, 5); // pending → 0
    session.setProgress(0.5); // entry mousemove at 0.5 (the resumed position)
    expect(session.getMaxProgress()).toBeCloseTo(0, 5); // 0.5 - 0.5
    session.setProgress(1.0); // sweep to end
    expect(session.getMaxProgress()).toBeCloseTo(0.5, 5); // 1.0 - 0.5
    session.destroy();
  });

  it('markRead resets coverage to 0', () => {
    const doc = setupDoc();
    const spans = makeSpans(doc, [0]);
    const session = createReadingSession(spans, null);
    session.setCurrent(0);
    session.setProgress(0); // enter at start
    session.setProgress(0.9); // coverage 0.9
    expect(session.getMaxProgress()).toBeCloseTo(0.9, 5);
    session.markRead(0);
    expect(session.getMaxProgress()).toBe(0);
    session.destroy();
  });
});
